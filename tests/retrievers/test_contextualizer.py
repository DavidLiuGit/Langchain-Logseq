...
class TestRetrieverContextualizer(unittest.TestCase):
    ...
    def test_init_with_valid_props(self):
        ...
        props = RetrieverContextualizerProps(
            llm=self.mock_llm, prompt=self.test_prompt, output_schema=None
        )
    ...
    def test_generate_chain_without_output_schema(self):
        ...
        self.assertIsInstance(
            ...
            type(
                PromptTemplate.from_template(self.test_prompt)
                | self.mock_llm
                | StrOutputParser()
            ),
        )
    ...
    def test_generate_chain_with_output_schema(self):
        ...
        props = RetrieverContextualizerProps(
            llm=self.mock_llm, prompt=self.test_prompt, output_schema=TestOutputSchema
        )
        ...
        assert isinstance(contextualizer.parser, PydanticOutputParser)
    ...
    def test_invoke_without_output_schema(self, mock_from_template):
        ...
        props = RetrieverContextualizerProps(llm=self.mock_llm, prompt=self.test_prompt)
    ...
    def test_prompt_template_with_schema(self):
        ...
        contextualizer = RetrieverContextualizer(props)
        prompt = contextualizer.prompt_template.format_prompt(
            user_input="What is Paris?"
        ).to_string()
