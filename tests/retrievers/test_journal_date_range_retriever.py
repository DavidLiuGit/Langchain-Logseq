...
class TestLogseqJournalDateRangeRetriever(unittest.TestCase):
    def setUp(self):
        ...
        self.mock_loader_input = LogseqJournalLoaderInput(
            journal_start_date=(datetime.now() - timedelta(days=7)).strftime(
                "%Y-%m-%d"
            ),
        ...
        self.retriever = LogseqJournalDateRangeRetriever(
            self.mock_contextualizer, self.mock_loader
        )
    ...
    def test_build_loader_input_type_error(self):
        ...
        self.mock_contextualizer.invoke.return_value = (
            "This is not a LogseqJournalLoaderInput"
        )
        ...
        self.assertIn(
            "Expected LogseqJournalLoaderInput but got", str(context.exception)
        )
    ...
    def test_get_relevant_documents_with_chat_history(self):
        ...
        # Execute
        result = self.retriever._get_relevant_documents(
            query, run_manager=mock_run_manager, chat_history=chat_history
        )
